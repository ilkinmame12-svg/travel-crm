...
export default function AssistantPage() {
  ...
  useEffect(() => {
    fetchBookings()
    if (messages.length === 0) setMessages([{ role: "assistant", content: "Salam! Mən itstour CRM köməkçisiyəm. Sifarişlər, maliyyə, borclar haqqında suallarınızı cavablaya bilərəm. Yeni sifariş əlavə etmək, axtarmaq və ya silmək üçün də kömək edə bilərəm! 🚀" }])
    setReady(true)
  }, [])
  ...
  function buildContext() {
    ...
    const managerStats = bookings.reduce((acc: any, b) => {
      ...
      acc[b.manager].count++; acc[b.manager].profit += b.profit; acc[b.manager].revenue += b.sellPrice; return acc
    }, {})
    ...
    <div className="min-h-screen p-5 md:p-7 flex flex-col" style={{ background: "var(--bg-primary)" }}>
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={() => { localStorage.removeItem('assistant_messages'); setMessages([{ role: "assistant", content: "Salam! Yenidən başlayırıq 👋" }]) }}
          className="flex items-center gap-2 px-3 py-2 rounded-2xl text-sm" style={{ background: "var(--bg-card)", border: "1px solid var(--border-color)", color: "var(--text-secondary)" }}>
          <Trash2 size={14} />Təmizlə
        </button>
      </div>

      <div className="flex-1 flex flex-col rounded-3xl overflow-hidden" style={{ background: "var(--bg-card)", border: "1px solid var(--border-color)", backdropFilter: "blur(20px)", minHeight: "500px" }}>
            ...
            <div className="flex gap-3">
              ...
              <div className="px-4 py-3 rounded-2xl" style={{ background: "var(--bg-glass)", border: "1px solid var(--border-color)", borderTopLeftRadius: "4px" }}>
                <div className="flex gap-1">
                  {[0, 150, 300].map(delay => <div key={delay} className="w-2 h-2 rounded-full animate-bounce" style={{ background: "var(--text-muted)", animationDelay: `${delay}ms` }} />)}
                </div>
              </div>
            </div>
          )}
        ...
        <div className="p-4" style={{ borderTop: "1px solid var(--border-color)" }}>
          <div className="flex gap-2 flex-wrap mb-3">
            {["Bu ay neçə sifariş var?", "Ən yaxşı menecer kim?", "Ödənilməmiş borclar?", "Yeni sifariş əlavə et"].map(q => (
              <button key={q} onClick={() => setInput(q)} className="text-xs px-3 py-1.5 rounded-xl transition-all" style={{ background: "var(--bg-glass)", border: "1px solid var(--border-color)", color: "var(--text-secondary)" }}>
                {q}
              </button>
            ))}
          </div>
