    ...
    <div className="min-h-screen bg-gray-50 p-6">
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        <div className="bg-green-600 rounded-2xl p-5 text-white">
          ...
          <p className="text-xs opacity-70 mt-1">Sifarişlər + əlavə gəlirlər</p>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 mb-4">
        <div className="flex gap-2 px-6 py-4 border-b border-gray-100">
          {[
            ...
            { value: "income", label: `💚 Gəlirlər` },
            { value: "expense", label: `🔴 Xərclər` },
          ].map(t => (
        ...
        {tab === "overview" && (
          <div className="grid grid-cols-2 divide-x divide-gray-100">
            <div className="p-6">
              <h3 className="text-sm font-semibold text-gray-700 mb-4">Son sifarişlər (gəlir)</h3>
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-100">
                    {["Müştəri", "İstiqamət", "Satış", "Mənfəət"].map(h => (
                ...
                <tbody>
                  {bookings.slice(0, 6).map(b => (
                    <tr key={b.id} className="border-b border-gray-50">
                      <td className="py-2 text-gray-900 font-medium">{b.clientName}</td>
                      <td className="py-2 text-gray-500">{b.destination}</td>
                      <td className="py-2 font-semibold">{formatCurrency(b.sellPrice)}</td>
                      <td className="py-2">
                        <span className={b.profit >= 0 ? "text-green-600 font-semibold" : "text-red-500"}>
        ...
        {tab === "income" && (
          <div className="p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-sm font-semibold text-gray-700">Əlavə gəlirlər</h3>
              <p className="text-sm font-bold text-green-600">Cəmi: {formatCurrency(totalManualIncome)}</p>
            </div>
            {incomes.length === 0 ? (
              <div className="text-center py-12 text-gray-400">
                <p className="text-sm">Hələ əlavə gəlir yoxdur</p>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-100">
                    {["Ad", "Açıqlama", "Tarix", "Məbləğ", ""].map(h => (
                ...
                <tbody>
                  {incomes.map(i => (
                    <tr key={i.id} className="hover:bg-gray-50 group border-b border-gray-50">
                      <td className="py-3 font-medium text-gray-900">{i.name}</td>
                      <td className="py-3 text-gray-500">{i.description}</td>
                      <td className="py-3 text-gray-500">{i.date}</td>
                      <td className="py-3 font-bold text-green-600">{formatCurrency(i.amount)}</td>
                      <td className="py-3">
                        <button onClick={() => setIncomes(prev => prev.filter(x => x.id !== i.id))}
      ...
      {modal === "income" && (
        <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            <div className="flex items-center justify-between px-6 py-4 border-b">
              <h2 className="text-lg font-semibold">Əlavə gəlir</h2>
              <button onClick={() => setModal(null)} className="text-gray-400 hover:text-gray-600 text-xl">✕</button>
            </div>
            <form onSubmit={handleAddIncome} className="p-6 space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Ad / Şirkət *</label>
                <input name="name" required placeholder="Məs: Anar Turizm" className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Məbləğ (AZN) *</label>
                <input name="amount" type="number" step="0.01" min="0" required className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Açıqlama</label>
                <input name="description" placeholder="Məs: Noyabr borcu ödənildi" className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Tarix</label>
                <input name="date" type="date" defaultValue={new Date().toISOString().split("T")[0]} className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500" />
              </div>
              <div className="flex justify-end gap-3 pt-2 border-t">
                <button type="button" onClick={() => setModal(null)} className="px-4 py-2 text-sm border border-gray-200 rounded-xl hover:bg-gray-50">Ləğv et</button>
                <button type="submit" className="px-5 py-2 text-sm bg-green-600 text-white rounded-xl hover:bg-green-700 font-medium">Əlavə et</button>
              </div>
            </form>
          </div>
        </div>
