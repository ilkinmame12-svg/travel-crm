...
export default function Sidebar() {
  ...
  return (
    <>
      {/* Overlay when expanded on desktop */}
      {expanded && <div className="fixed inset-0 z-30" onClick={() => setExpanded(false)} />}

      <div
        className="hidden md:flex flex-col fixed top-0 left-0 h-full z-40 transition-all duration-300"
        style={{
      ...
      >
        {/* Logo / Toggle */}
        <div className="flex items-center justify-between px-3 py-4" style={{ borderBottom: "1px solid var(--border-color)", minHeight: "60px" }}>
          {expanded ? (
